...
core 是核心部分，最开始将很多东西都打包进去了，但是仔细思考下，可以将很多东西都剔除掉
...
common 是一些公共函数或者库的部分，包括一些简单好用的工具函数等，用来提供一些开发过程的简化操作

### cli

cli 工具是用来快速初始化相关的命令，包括快速创建一个模块、一个 service 等

## 后续升级

### 动态加载

当前模块会在初始化的时候进行加载，这样虽然说静态依赖比较好分析，但是会导致的问题就是，会陷入循环依赖，当前尚未完全了解清楚循环依赖的机制，但是目前来看频率还挺高的

动态加载的话，需要修改框架，使其成为可动态运行，也就是说提供一些导入和卸载的 api

此外需要考虑到去中心化的设计，避免所有的状态都集中在一起, 导致相关的插件无法及时处理
...
1. 将 module 以 child --> next --> parent 的方式进行组织
2. 将 module 的属性存放在 class 中，全部为整体所用
3. 将 module 的使用属性存放在特定解析中，让业务自行取用

appModule --> Yuan --> moduleFactory --> middlewareFactory --> render
原始来源 导入进来 经过模块工厂处理 导入给插件处理 react 渲染

问题点：动态模块怎么注册？
模块不在框架上注册，避免动态注册模块导致整体重新 render，提供一个动态模块的组件，渲染时通过添加这个组件，来进行内容的加载
而这个模块不在整体链路上面，仅做注册
